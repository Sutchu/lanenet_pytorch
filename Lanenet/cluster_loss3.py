...
class cluster_loss(nn.Module):
    ...
    def forward(self, binary_logits, binary_labels,
        ...
        instance_segmenatation_loss = torch.tensor(0.).to(device = 'cuda')#.cuda()
        ...
        for dimen in range(batch_size):
            ...
            prediction = torch.unsqueeze(instance_logits[dimen], 0).to(device = 'cuda')#.cuda()
            correct_label = torch.unsqueeze(instance_labels[dimen], 0).to(device = 'cuda')#.cuda()
            instance_segmenatation_loss += instance_loss(prediction, correct_label, delta_v, delta_d)
